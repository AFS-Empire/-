 ...
 * 加密与 PIN 校验模块
 *
 * 1. PBKDF2-SHA256 加盐哈希：替代旧的简单字符串哈希，工业标准强度
 * 2. 动态 PIN：网络标准时间 + 私有盐 → SHA-256 → 6位截断
 *
...
/**
 * 私有盐 —— 分散在代码多处拼接，非单一硬编码常量
 * 注意：纯前端环境下盐本身无法保密，但配合时间因子可让静态分析无法预测未来 PIN
 */
const PIN_SALT_PARTS = [
  'AFS-JZY',        // 创作者署名
  '1360335662',     // QQ
  'ophis',          // 项目代号
  'archive',        // 模块标识
];
const PIN_SALT = PIN_SALT_PARTS.join('::');

/** PIN 有效期（秒）—— 30秒换一次 */
export const PIN_TTL_SECONDS = 30;

/** 网络时间缓存：避免每次校验都发请求 */
let cachedNetworkTime: number | null = null;
let cachedAt = 0;
let lastLocalOffset = 0; // 网络时间 - 本地时间 的偏移

/**
 * 获取网络标准时间（优先 worldtimeapi，失败回退到本地时间）
 * 返回 Unix 秒级时间戳
 */
export async function fetchNetworkTime(): Promise<{
  time: number;
  ...
  // 5分钟内用缓存
  const now = Date.now();
  if (cachedNetworkTime !== null && now - cachedAt < 5 * 60 * 1000) {
    const extrapolated = cachedNetworkTime + (now - cachedAt);
    return { time: Math.floor(extrapolated / 1000), source: 'cached', offset: lastLocalOffset };
  }
  ...
  try {
    ...
    if (!networkMs || isNaN(networkMs)) throw new Error('invalid datetime');

    cachedNetworkTime = networkMs;
    cachedAt = now;
    lastLocalOffset = networkMs - now;
    return { time: Math.floor(networkMs / 1000), source: 'network', offset: lastLocalOffset };
  } catch {
    // 断网或超时：用本地时间 + 上次偏移（如果有）
    const localMs = now + lastLocalOffset;
    cachedNetworkTime = localMs;
    cachedAt = now;
    return { time: Math.floor(localMs / 1000), source: 'local', offset: lastLocalOffset };
  }
}

/**
 * 计算当前有效 PIN
 * @param timeSec Unix 秒级时间戳
 * @returns 6位数字 PIN
 */
export async function computePin(timeSec: number): Promise<string> {
  // 按 PIN_TTL_SECONDS 对齐时间窗口
  const window = Math.floor(timeSec / PIN_TTL_SECONDS);
  const input = `${window}::${PIN_SALT}`;
  const bytes = strToBytes(input);
  // 用完整 SHA-256（兼容回退），取前4字节做6位数字
  const hash = await sha256(bytes);
  const dv = new DataView(hash.buffer);
  const num = dv.getUint32(0, false);
  const pin = num % 1000000;
  return pin.toString().padStart(6, '0');
}

/**
 * 获取当前有效 PIN（含来源信息，供界面展示）
 */
export async function getCurrentPin(): Promise<{ pin: string; source: string; expiresAt: number }> {
  const { time, source } = await fetchNetworkTime();
  const pin = await computePin(time);
  // 当前窗口结束时间 = 下一个窗口起点
  const window = Math.floor(time / PIN_TTL_SECONDS);
  const expiresAt = (window + 1) * PIN_TTL_SECONDS * 1000;
  const sourceLabel = source === 'network' ? '网络时间' : source === 'cached' ? '缓存时间' : '本地时间(断网)';
  return { pin, source: sourceLabel, expiresAt };
}

/**
 * 校验用户输入的 PIN 是否匹配当前有效窗口
 * 允许 ±1 窗口容差（防止边界时刻刚好换 PIN 导致误判）
 */
export async function verifyPin(input: string): Promise<{ valid: boolean; reason?: string }> {
  if (!/^\d{6}$/.test(input)) {
    return { valid: false, reason: 'PIN 必须是 6 位数字' };
  }
  const { time, source } = await fetchNetworkTime();
  const window = Math.floor(time / PIN_TTL_SECONDS);

  // 检查三个窗口：当前、上一个、下一个
  for (const w of [window, window - 1, window + 1]) {
    const expected = await computePin(w * PIN_TTL_SECONDS);
    if (timingSafeEqual(input, expected)) {
      return { valid: true };
    }
  }
  const reason = source === 'local'
    ? 'PIN 错误（当前为本地时间，断网状态下容错性降低）'
    : 'PIN 错误或已过期';
...
/**
 * 检测系统时间是否被篡改
 * 通过比较本次网络时间和"上次网络时间 + 经过时间"的差值
 * 返回 true 表示疑似篡改
 */
export async function detectClockTampering(): Promise<{ tampered: boolean; detail: string }> {
  const { source, offset } = await fetchNetworkTime();
  if (source !== 'network') {
    return { tampered: false, detail: '断网无法校验时钟' };
  }
  // 偏移超过 5 分钟视为可疑
  if (Math.abs(offset) > 5 * 60 * 1000) {
    return {
      tampered: true,
      detail: `系统时间与网络时间偏差 ${Math.round(offset / 1000 / 60)} 分钟，疑似篡改本地时钟`,
    };
  }
  return { tampered: false, detail: '时钟正常' };
}
