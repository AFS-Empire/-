...
const READER_THEMES: Record<ReaderTheme, ReaderThemeConfig> = {
  dark: { label: '深色', swatch: '#0c0c0c', accentColor: 'var(--color-gold-500)' },
  light: { label: '浅色', swatch: '#f7f2e6', accentColor: 'var(--color-gold-500)' },
  parchment: { label: '牛皮纸', swatch: '#dcc896', accentColor: 'var(--color-gold-500)' },
};
...
export default function NovelReader() {
  ...
  const initial = useRef<ReaderSettings>(() => {
    const saved = loadReaderSettings();
    const globalTheme = localStorage.getItem('theme') as ReaderTheme | null;
    if (globalTheme && READER_THEMES[globalTheme]) {
      return { ...saved, theme: globalTheme };
    }
    return saved;
  });
  ...
  // 同步全局主题：切换 html 上的 light/parchment 类（挂载时自动执行）
  useEffect(() => {
    const root = document.documentElement;
    root.classList.remove('light', 'parchment');
    if (readerTheme === 'light') {
      root.classList.add('light');
    } else if (readerTheme === 'parchment') {
      root.classList.add('parchment');
    }
    localStorage.setItem('theme', readerTheme);
  }, [readerTheme]);
  ...
  const theme = useMemo(() => ({
    ...
    titleColor: 'var(--color-gold-400)',
    accentAlpha: (hex: string) => {
      const alpha = parseInt(hex, 16) / 255;
      return `color-mix(in srgb, var(--color-gold-500) ${Math.round(alpha * 100)}%, transparent)`;
    },
  }), []);
      ...
      if (progress && progress.lastChapterId === chapterId) {
        requestAnimationFrame(() => {
          ...
          if (el) {
            ...
            const target = progress.scrollRatio * Math.max(0, max);
            el.scrollTop = target;
          }
        });
      }
  ...
  return (
    <div className="max-w-2xl mx-auto px-2 sm:px-0 pb-20">
      ...
      <div
        ...
        style={{ backgroundColor: theme.accentAlpha('60') }}
      />
            ...
            <div className="py-2">
              {bookChapters.map((c, i) => (
                <button
                  ...
                  style={{
                    backgroundColor: c.id === chapterId ? theme.accentAlpha('18') : 'transparent',
            ...
            <div className="p-4 space-y-6">
                    ...
                    return (
                      <button
                        ...
                        style={{
                          ...
                          backgroundColor: active ? `color-mix(in srgb, ${t.accentColor} 8%, transparent)` : 'transparent',
                        }}
                      >
                        <span
                ...
                <div className="flex gap-1.5">
                  {(Object.keys(FONT_SIZES) as FontSize[]).map(s => (
                    <button
                      ...
                      style={{
                        backgroundColor: fontSize === s ? theme.accentAlpha('20') : 'transparent',
                ...
                <div className="flex gap-2">
                  {(Object.keys(LINE_HEIGHTS) as LineHeight[]).map(l => (
                    <button
                      ...
                      className="flex-1 py-2 rounded text-sm transition-all"
                      style={{
                        backgroundColor: lineHeight === l ? theme.accentAlpha('20') : 'transparent',
              ...
              {book?.spoilerMode === 'unlock' && !isSpoilerUnlocked && (
                <div
                  ...
                  style={{
                    backgroundColor: theme.accentAlpha('10'),
                    border: `1px solid ${theme.accentAlpha('30')}`,
      ...
      {activeChar && (
        ...
        >
          <div
            ...
            style={{ backgroundColor: 'var(--bg-surface)', border: `1px solid ${theme.accentAlpha('40')}` }}
            onClick={e => e.stopPropagation()}
          >
