...
### 8. 数据安全与密钥体系（强制规定）
**私有密钥** = App 专有盐值（**严禁明文写死源码、严禁上传 GitHub**）。
- 盐值仅存于两处：本地 `.env.local`（已被 `.gitignore` 忽略）+ GitHub Actions Secrets 的 `ARCHIVE_PRIVATE_SALT`。
- 构建时由 `vite.config.ts` 从环境变量读取并内联为常量；网页构建经 vite alias 替换为空字符串，产物中不存在。
- 当前生效盐值（2026-08-02 轮换）：通过上述安全渠道分发，不写入本文件。

#### 8.1 盐值轮换硬性规则（强制规定）
1. **旧盐加密的备份文件，新版程序打不开**：切换盐值后，旧版导出的加密备份无法被新版解密/验签。
2. **切换盐之前必须先完成数据迁移**：
   - 步骤1：用**旧版 App** 导入所有已有数据 → 全部重新导出一遍（生成旧格式备份留底）。
   - 步骤2：部署/安装**新版 App**（已切换为新盐）→ 用新盐重新导出所有数据（生成新格式备份）。
   - 步骤3：确认新版能正常导入新格式备份后，迁移完成。旧格式备份在新版中无法打开属预期行为。
3. **盐值变更必须同步更新两处**：本地 `.env.local` + GitHub Secrets 的 `ARCHIVE_PRIVATE_SALT`，否则本地构建与 CI 构建产物盐值不一致会导致备份互不兼容。
4. **轮换记录**：2026-08-02 完成首次盐值轮换（旧盐值已废弃，仅留存于 git 历史无法清除；新盐值经安全渠道分发）。

#### 8.2 敏感数据日志禁令（强制规定）
**严禁**通过 `console.log/info/warn/error/debug` 打印盐、密钥、签名密钥、调试密钥哈希等敏感变量（含前缀/后缀截断片段），防止打包后运行时意外泄露。
- 已审计清除：`src/debug/devTools.ts`（移除调试密钥哈希的 `console.info`）、`scripts/verify-web-secrets.js`（移除盐值前6后4片段打印）。
- `src/data/db.ts` 中水印作者名属用户可见元数据，非盐/密钥类敏感变量，保留。
- 仅构建机器运行、不进入任何网页/App 产物的诊断脚本，也只能打印长度等脱敏信息，不得打印盐值内容。
