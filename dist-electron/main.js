...
function resolvePublicAsset(relPath) {
    ...
    return path.join(electron_1.app.getAppPath(), 'public', relPath);
}
/** 确保目录存在（同步创建） */
 ...
 */
electron_1.ipcMain.handle('backup:save', async (_event, payload) => {
    if (!mainWindow)
    ...
    const res = await electron_1.dialog.showSaveDialog(mainWindow, opts);
...
/** 主进程里集中读取备份文件 */
electron_1.ipcMain.handle('backup:load', async (_event) => {
    if (!mainWindow)
        return { ok: false, error: '窗口不存在' };
    const opts = {
        title: '导入档案备份',
    ...
    const res = await electron_1.dialog.showOpenDialog(mainWindow, opts);
...
function writeAutoSnapshot(json, opts = {}) {
    try {
...
function cleanOldAutoBackups() {
    ...
    for (const f of toDelete) {
        try {
            fs.unlinkSync(path.join(autoBackupDir(), f.name));
        }
        catch { /* 忽略 */ }
    }
}
/** 渲染层每次数据变化推送快照 → 主进程落盘 */
electron_1.ipcMain.handle('backup:auto-snapshot', (_event, payload) => {
    writeAutoSnapshot(payload.json, { forceStamp: false });
    return { ok: true };
});
/** 列出所有自动备份（含 latest + 历史时间戳） */
electron_1.ipcMain.handle('backup:list-auto', () => {
    return { ok: true, dir: autoBackupDir(), items: listAutoBackups() };
});
/** 从指定自动备份文件读取 JSON 返回给渲染层 */
electron_1.ipcMain.handle('backup:restore-auto', (_event, payload) => {
    try {
...
electron_1.ipcMain.handle('backup:delete-auto', (_event, payload) => {
    try {
        const dir = autoBackupDir();
        const full = path.normalize(path.join(dir, payload.name));
        if (!full.startsWith(dir))
            return { ok: false, error: '非法路径' };
        if (payload.name === 'latest.json')
            return { ok: false, error: 'latest 不可删除' };
        if (fs.existsSync(full))
            fs.unlinkSync(full);
        return { ok: true };
    }
...
electron_1.app.whenReady().then(() => {
    ...
    ensureDir(path.join(electron_1.app.getPath('userData'), 'backups'));
    createMainWindow();
...
electron_1.ipcMain.handle('backup:final-snapshot', (_event, payload) => {
    writeAutoSnapshot(payload.json, { forceStamp: true });
    pendingSnapshot = false;
    // 给一点时间让 fs.writeFileSync 完成，然后退出
    setTimeout(() => electron_1.app.quit(), 100);
    return { ok: true };
});
// 渲染层崩溃白屏兜底：记录错误
